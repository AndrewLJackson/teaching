...
$$W_{s} = W_{b} - 0.2 * W_{b}$$ $$W_{s} = W_{b} * (1 - 0.2)$$ $$W_{s} = W_{b} *
0.8$$ $$\frac{W_{s}}{0.8} = W_{b}$$ $$1.25 * W_{s} = W_{b}$$

But we know from our hypothetical example above that $W_{s} = 0.5$ so $$W_{b} =
1.25 * 0.5$$ $$W_{b} = 0.625$$

So, how does productivity of biparental care $W_{b}$ compare with a fully 
asexual $W_{a}$ strategy? $$\frac{W_{a}}{W_{b}} = \frac{1}{0.625} = 1.6$$
